/* DebugAI analytics — PostHog.
   Loads only when POSTHOG_KEY is present in window.__DEBUGAI_CONFIG__. */
(function () {
  var cfg = window.__DEBUGAI_CONFIG__ || {};
  if (!cfg.posthogKey) return;

  /* PostHog official snippet (2024) */
  ...
  window.debugaiIdentify = function(userId, traits) {
    try { posthog.identify(String(userId), traits || {}); } catch(_) {}
  };
})();
